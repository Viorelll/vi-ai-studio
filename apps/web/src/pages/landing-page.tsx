...
export function LandingPage() {
  ...
  const recentGenerated = useGeneratedProjectsWithVersions(generatedProjects.slice(0, 3));
  ...
  const readyCount = generatedProjects.filter((s) => s.status === "ready").length;
  const failedCount = generatedProjects.filter((s) => s.status === "failed").length;
  ...
  return (
    <main className="flex-1 flex justify-center px-7 pb-12 relative overflow-hidden">
      <div
        ...
        style={{ background: "linear-gradient(180deg, var(--background) 0%, var(--brand-tint) 70%, var(--brand-tint) 100%)" }}
      />
      <div className="w-full max-w-[1180px] relative z-10">
        <div className="flex items-end justify-between gap-10 flex-wrap py-14">
          <div className="max-w-[520px]">
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
              AI Build Platform
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight mb-3">Vi - AI Studio</h1>
          ...
          <div className="flex gap-5 flex-wrap">
            <Card size="sm" className="px-5 py-4">
              ...
              <CardContent className="p-0 flex gap-5">
                ...
                <Stat label="Building" value={buildingCount} className="text-primary" />
              </CardContent>
            </Card>
            <Card size="sm" className="px-5 py-4 bg-[var(--brand-tint)] border-[var(--brand-tint-border)]">
              <CardHeader className="p-0">
              ...
              <CardContent className="p-0 flex gap-5">
                ...
                <Stat label="Ready" value={readyCount} className="text-green-700 dark:text-green-500" />
                <Stat label="Failed" value={failedCount} className="text-red-700 dark:text-red-500" />
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-14">
          <RecentList title="Recently generated project specifications" items={specs.slice(0, 3)} />
          <RecentGeneratedList title="Recently generated projects" projects={recentGenerated.data} />
        </div>
      </div>
    </main>
  );
...
function Stat({ label, value, className }: { label: string; value: number; className?: string }) {
  return (
    <div className="text-center min-w-[56px]">
      <div className={`text-[22px] font-extrabold tracking-tight ${className ?? ""}`}>{value}</div>
...
function NavCard({
  ...
  return (
    <Link to={to} className="group">
      <Card
        className={`h-full p-7 gap-3.5 transition hover:shadow-md hover:ring-foreground/20 ${
          tinted ? "bg-[var(--brand-tint)] border-[var(--brand-tint-border)]" : ""
        }`}
      >
        ...
        <div className="text-[13px] font-semibold text-primary">View table →</div>
      </Card>
    </Link>
  );
}
...
function RecentGeneratedList({
  ...
  title: string;
  projects: { spec: SpecificationSummary; generations: SpecificationDetail["generations"] }[];
}) {
  return (
    <div>
      ...
      <Card className="p-0 overflow-hidden">
        {projects.length === 0 ? (
          <div className="px-4.5 py-4 text-sm text-muted-foreground">Nothing here yet.</div>
        ) : (
          projects.map(({ spec, generations }) => (
            <RecentGeneratedItem key={spec.id} spec={spec} generations={generations} />
          ))
        )}
      </Card>
    </div>
...
function RecentList({ title, items }: { title: string; items: SpecificationSummary[] }) {
  return (
    <div>
      <div className="text-[13px] font-semibold text-muted-foreground mb-3">{title}</div>
      <Card className="p-0 overflow-hidden">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <Link
              ...
              className="flex items-center justify-between px-4.5 py-3.5 border-b last:border-b-0 hover:bg-muted/50"
            >
              <span className="text-[13px] font-semibold">{item.name}</span>
              <div className="flex items-center gap-3.5">
                <span className="text-xs text-muted-foreground">{new Date(item.created).toLocaleDateString("en-US")}</span>
...
function SpecIcon({ className, compact }: { className?: string; compact?: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
